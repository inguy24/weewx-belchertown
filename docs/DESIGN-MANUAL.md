...
- **Tile-chart exception:** Charts inside tile-footprint (1-column) cards use `--text-chart-label-sm` (11px) for tick labels, `XAxis height={15}`, and tighter margins (`{ top: 2, right: 12, bottom: 0, left: 12 }`). This is a space constraint — tile content areas cannot accommodate 14px labels with standard margins. Standard-size cards (wide, panel, full) use the normal 14px / `--text-chart-label`.
...
- **Typography:** Axis labels, ticks, and data labels use Lexend (`--font-chart`) at `--text-chart-label` size. Recharts `tick={{ fontSize }}` props take pixel values — use 14px (the pixel equivalent of 0.875rem). Do not use rem values in Recharts tick props. **Tile-chart exception:** charts inside tile-footprint cards use 11px (`--text-chart-label-sm`), `XAxis height={15}`, and tighter margins (`{ top: 2, right: 12, bottom: 0, left: 12 }`) — the standard 14px labels are too large for the constrained content area of a 1-column tile.
