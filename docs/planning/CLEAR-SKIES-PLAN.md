...
**Status:** Phase 1 (architecture & contracts) — in progress. ADR backlog closed (40 ADRs Accepted as of 2026-05-05); tech-stack spike complete; weather-dev LXD container stood up; docker-compose dev/test stack scaffolded + validated; OpenAPI contract committed. Remaining Phase 1: canonical data model spec, security baseline, 5 GitHub repos + auto-sync, CI scaffolding.
...
| Canonical internal data model | ✅ | [ADR-010](../decisions/ADR-010-canonical-data-model.md) — Accepted. 8 entity types + 2 containers; weewx-aligned camelCase JSON / snake_case Python; units = weewx target_unit with metadata block; single-station model; prose at three layers (weatherText, narrative, ForecastDiscussion) |
...
| Stand up the 5 GitHub repos | ⬜ | Named per [ADR-004](../decisions/ADR-004-repo-naming.md), licensed per [ADR-003](../decisions/ADR-003-license.md), with `README` + `LICENSE` + `SECURITY.md` placeholders |
| Wire CI scaffolding | ⬜ | GitHub Actions: lint + test on PR; release workflow on tag |
| Security baseline document | ⬜ | Codify cross-cutting controls into a checklist. Output: `docs/contracts/security-baseline.md` |
