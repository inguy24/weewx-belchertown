...
5. **Chart tick fontSize:** Standard cards (`current-conditions-card.tsx`, `ConfigDrivenChart.tsx`, `MonthlyAveragesCard.tsx`, `WeatherRangeChart.tsx`) use 14px per `--text-chart-label`. **Tile-footprint cards** (`solar-radiation-card.tsx`, `uv-index-card.tsx`, `lightning-card.tsx`) use 11px per `--text-chart-label-sm` with `XAxis height={12}` and tighter margins `{ top: 2, right: 8, bottom: 0, left: 8 }` — 14px labels are too large for tile content areas. (Design manual updated 2026-06-17.)
