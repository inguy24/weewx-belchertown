...
4. **Weather range chart:** custom SVG polar chart showing daily temperature range. Uses dual `agg` fetch (`min` + `max`) via the `/archive?agg=min|max` parameter.
...
7. **Per-field aggregation (2026-06-07):** Each series in `charts.conf` may specify `aggregate_type` (e.g., `sum` for rainTotal, `max` for rainRate). The dashboard passes these to the API via an `agg_map` query parameter. Fields without an explicit type default to `AVG` (Belchertown's rolling-range default). This matches Belchertown's per-series aggregation from `graphs.conf`.
...
- `src/components/charts/WeatherRangeChart.tsx` — custom SVG polar temperature range
