...
UI-related ADRs below have been consolidated into [`docs/DESIGN-MANUAL.md`](../DESIGN-MANUAL.md), which is now the single authority for UI design rules. These archived ADRs preserve the historical decision process (the *why*) but are not consulted for current rules. Archived copies are in [`docs/archive/decisions/`](../archive/decisions/).
...
| [ADR-051](../archive/decisions/ADR-051-card-footprint-model.md) | Card footprint model & grid-compatible sizing | 2026-06-16 | 2026-05-30 |
| [ADR-062](../archive/decisions/ADR-062-card-header-contract.md) | Card header contract | 2026-06-16 | 2026-06-16 |

> **ADR-024** (page taxonomy) has both UI and non-UI content (route definitions, footer structure). It remains in the active ADR table; its UI rules have been extracted into the design manual.
...
- **Archived** — UI rules consolidated into DESIGN-MANUAL.md. Historical decision rationale preserved.
...
6. **UI decisions:** After acceptance, extract rules into `docs/DESIGN-MANUAL.md`, then archive the ADR to `docs/archive/decisions/`.
