...
3. `docs/manuals/PROVIDER-MANUAL.md` §5 — AQI provider rules, pass-through architecture, anti-patterns
4. `docs/manuals/API-MANUAL.md` §2 — data model, AQIReading entity, response shapes
5. `docs/manuals/DASHBOARD-MANUAL.md` — Now page card inventory, card plugin system
...
| `docs/manuals/PROVIDER-MANUAL.md` §5 | Add sub-section on per-pollutant sub-index pass-through. Update IQAir section: VERIFIED, add concentration fields, note µg/m³ units, note Startup+ requirement. |
| `docs/manuals/DASHBOARD-MANUAL.md` | Add note: AQI card dots colored per-pollutant from `pollutantSubIndices` when available; fall back to overall AQI color when absent. |
