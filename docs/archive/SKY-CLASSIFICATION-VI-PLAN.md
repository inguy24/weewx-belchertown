...
**Status:** COMPLETE (superseded) — VI system was implemented, then the Kv-first decision tree (SKY-CLASSIFICATION-KV-FIRST-PLAN, ADR-073) replaced the Km-first approach. The core CAELUS infrastructure (ring buffer, 1-minute binning, four indices, temporal coherence filter, startup backfill) from this plan persists; only the classification decision tree was reworked. Archived 2026-06-27.
**Created:** 2026-06-18
...
- Files:
  ...
  - `docs/manuals/API-MANUAL.md` line 549 — remove "Davis 6450 pyranometer ±5%" sentence
...
- Do: Grep all manuals for sky condition / σ / Davis 6450 references:
  - `docs/manuals/DESIGN-MANUAL.md` — sky background mapping (lines 409–410) uses NWS labels ("Clear", "Mostly Cloudy", etc.) which are unchanged. No changes needed.
  - `docs/manuals/DASHBOARD-MANUAL.md` — scene descriptor references only. No classification methodology. No changes needed.
  - `docs/manuals/OPERATIONS-MANUAL.md` — no sky classification references. No changes needed.
  - `docs/manuals/PROVIDER-MANUAL.md` — cloud cover mentioned only in astronomy context. No changes needed.
