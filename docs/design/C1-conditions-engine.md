...
**`weewx-clearskies-realtime`** — the realtime service (formerly the BFF, now merged into the API per ADR-058).
...
- **ADR-041** (`ADR-041-realtime-bff.md`) — originally established the former realtime service's enrichment role (now merged into the API per ADR-058); explicitly lists the built components: "proxy, units module, sky condition, conditions text, temperature comfort, enrichment pipeline (input smoother, ring buffer, barometer trend, **weather text**, sky tap)." Confirms ~5,000 LOC of production code (all now in the API repo).
...
**REST path:** Dashboard calls `GET /api/v1/current` → Caddy proxies to API → API runs enrichment pipeline (`enrich_barometer_trend`, `enrich_weather_text`, `enrich_uv`, `enrich_scene` in order) → API returns enriched JSON to dashboard.
...
- `weatherText` (the composed human-readable string) is available via `GET /api/v1/current` → `data.weatherText`, fully populated by the API enrichment pipeline.
...
5. **Weather description text** — `weatherText` prop (plain string) when truthy. Priority: `observation.weatherText` (API-composed conditions sentence) falls back to `todayForecast.weatherText` (provider forecast text) falls back to `null` (nothing rendered).
6. **Dewpoint** — `observation.dewpoint` as a `ConvertedValue`.
7. **Humidity** — `observation.outHumidity` as a `ConvertedValue`.
8. **Comfort index** — `observation.comfortIndex` from API (ADR-042). Renders either wind chill or heat index tile; the `comfortIndex` field is `"windChill" | "heatIndex" | "none"` — no client-side threshold logic.

### Key design point

`weatherText` and `weatherCode` are two separate things in the card. The text (the human-readable sentence from the API engine) is the C1 concern. The icon (`weatherCode`) is a forecast-provider integer code from the daily forecast. They can evolve independently.
...
```python
weatherText is always null in the base endpoint response; the API
enrichment pipeline populates it before serving the dashboard.
```

> **Note:** This docstring may still reference "BFF" in the actual code — it was written when the enrichment pipeline lived in the separate realtime service. The enrichment pipeline now runs within the API (ADR-058).

And again in the endpoint handler docstring:

```python
weatherText is always null here; the API enrichment pipeline populates
it per ADR-041/ADR-058 before serving the dashboard.
```

**Why this design:** The conditions text engine requires state — the 30-minute kc rolling buffer, the smoothed sensor ring buffers, and the weather pattern hold-timer. These live in the API enrichment pipeline which runs continuously and accumulates loop packets. The base API endpoint is stateless per-request and reads only the archive DB. The enrichment pipeline intercepts every `/api/v1/current` response, injects `weatherText` from its own engine, and returns the enriched response to the browser. The raw endpoint's `null` is the correct handoff signal.
...
2. **Implied: the `/current` endpoint handler is the only place to look for the conditions text.**
   Wrong. The `/current` endpoint handler returns raw observation data. The conditions engine (`conditions_text.py`, `sky_condition.py`, `temperature_comfort.py`, `enrichment/weather_text.py`) runs as a post-processing step within the API's enrichment pipeline — it enriches the response after the handler produces it. They are separate layers within the API; the handler code has no direct dependency on the conditions engine code. Reading only the endpoint handler misses the enrichment pipeline entirely.
...
1. **`weatherText` is REST-only, not SSE.** ADR-041 describes the enrichment pipeline but does not explicitly state that `weatherText` is excluded from the SSE loop packet field map. A reader of the ADR alone could assume the SSE stream carries `weatherText`; it does not. The field updates on REST poll cadence only. This is a documentation gap that could matter for C1's "live update" design decisions.

2. **`weatherCode` is forecast-sourced, not observation-sourced.** The card prop is `todayForecast?.weatherCode`. There is no `weatherCode` field on the `Observation` interface. If C1 needs to derive the icon from local conditions (rather than the daily forecast provider's code), a new mapping from the API-composed sky label to a WMO-style code would be required — that is currently not built. The as-built card has an icon only when a forecast provider is configured and returns `weatherCode`.
...
| **Section** | "Realtime (API) endpoints" table (lines ~220–228) |
| **What it says** | Lists `/api/v1/*` (proxy) and `/sse` (SSE stream) as the only API endpoints. Describes API as doing "unit conversion," "barometer trend," "weather text" in the ADR-041 consequences bullet (copied verbatim). But nowhere in the doc is there a section explaining what the conditions engine is, what it inputs, what it outputs, or how `weatherText` gets into the `/current` response. |
| **Reality** | The conditions engine is a fully built, multi-module, stateful subsystem in the API: `conditions_text.py`, `sky_condition.py`, `temperature_comfort.py`, `enrichment/weather_text.py`, `enrichment/input_smoother.py`. It is registered as an enrichment for every `GET /api/v1/current` response. It produces `data["data"]["weatherText"]`. |
...
| **Reality** | The API deliberately returns `weatherText: null` on every `/current` response from the base endpoint. The API enrichment pipeline injects the real value. This is a notable intentional-by-design point that caused the prior research agent to misread the system. |
| **Recommended fix** | Add a note to the `/api/v1/current` row: "The `weatherText` field is always null in the raw API response; the API enrichment pipeline (`enrich_weather_text`) injects the composed conditions string before serving the dashboard (ADR-041, ADR-044)." |
| **Class** | REFERENCE |
...
| **Reality** | The engine IS built and wired. `weatherText` is null in the raw API response deliberately (by design per ADR-041) — the base endpoint always returns null, then the API enrichment pipeline writes the real value. The field is not null because the engine is "off" — it is null at the base endpoint layer because the enrichment pipeline runs as a post-processing step. When the API is running and has data, the field is populated. The engine does NOT require a forecast provider to produce output — its primary inputs are local sensors (radiation, rainRate, windSpeed, appTemp, dewpoint). The forecast provider is only used as a sky-condition fallback when solar analysis is unavailable. |
| **Recommended fix** | Replace the Notes cell with: `"Always null in the raw API response (by design, ADR-041). The API enrichment pipeline injects the composed conditions string before serving the dashboard. Not from weewx archive — produced by the API conditions engine (conditions_text.py). Non-null when the API enrichment pipeline is running and has at least ~3 minutes of loop packet data. A forecast provider is only needed for night-time sky classification; daytime classification and all other components (precip, wind, comfort) are local-sensor-only."` |
| **Class** | REFERENCE |
...
| **Reality** | The blending engine IS fully wired (see `__main__.py` lines 283–284, `enrichment/weather_text.py`, git log commits `e9dbdd7` through `eafb706`). The correct statement is: `weatherText` is null in the raw API response (by design), but the API enrichment pipeline injects a composed string from the conditions engine before the dashboard sees it. The card must handle null for the edge case of insufficient loop data (~3 min startup window) or no enrichment pipeline running, not because the engine is unwired. |
| **Recommended fix** | Replace bullet with: `"weatherText is produced by the API conditions engine and injected into every /current response. It may be null during the ~3 minute startup window (insufficient solar kc data) or when the API enrichment pipeline is not configured. Design for the null state as a brief/edge case, not the normal state."` |
| **Class** | REFERENCE |
...
| **Reality** | The cross-check note correctly quotes the API docstring but then misinterprets it. The API enrichment pipeline IS implemented and running — it is not a future step. The "always null in API responses" clause is a statement about where the field is null (the base endpoint layer), not about whether the feature exists. The enrichment pipeline writes the non-null value. The OpenAPI omission question is still valid but the reasoning ("always null today") is factually wrong. |
| **Recommended fix** | Replace the interpretation paragraph with: `"The API enrichment pipeline is fully built and running (conditions_text.py, sky_condition.py, enrichment/weather_text.py). The base endpoint always returns null; the enrichment pipeline replaces it. This is intentional-by-design (ADR-041), not a future step. The OpenAPI omission question remains valid independently: does the published openapi-v1.yaml need to list weatherText on the Observation schema if the enrichment pipeline always populates it before the browser sees it? That is a contract question for the lead to resolve — the engine itself is not the issue."` |
| **Class** | REFERENCE |
...
- **ADR-041** explicitly describes the conditions engine as part of the former realtime service's as-built scope (now merged into the API per ADR-058). No contradiction.
