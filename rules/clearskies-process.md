...
- **Lead independently re-runs the verification command(s) before accepting the teammate's pytest claim. The teammate's self-report is one data point, not the truth.** Re-run the same command from a fresh shell; compare the pass/fail counts. If they match, proceed. If they don't, the teammate's claim was either a different selection (narrower file list, `-x` early stop, marker filter, etc.) or stale (run before their last commit). Surface the discrepancy before any lead-direct work begins. **Why (2026-05-11):** 3b-12 close — api-dev claimed "Full non-integration suite: 1762 passed, 0 failed"; lead's independent full-suite verification on the identical commit returned **103 failed / 1754 passed / 286 skipped**. 102 of the 103 failures were pre-existing (failed at `81600f8` too, before any 3b-12 work) and test-author had explicitly flagged them in their closeout — the lead initially trusted the api-dev count and almost let the round close on a false-clean pytest narrative. Independent verification surfaced both the inherited failure load (kicked to a parking-lot for triage in a future round) AND a small 3b-12-specific test-isolation regression that resolved with the F2 lead-direct delete. **How to apply:** at Step H gate (post-spawn, pre-audit), re-run the teammates' claimed pytest commands yourself; if the counts diverge from teammate-reported, investigate before treating the round as ready for audit. Cheap (~one extra ssh+pytest invocation); catches the false-clean narrative class of failure that no later step will catch.
...
### Sub-rule — api-docs file provenance is part of the cross-check

When an api-docs file at `docs/reference/api-docs/<provider>.md` was written from upstream documentation (vendor docs, blog posts, third-party examples) rather than from a **live capture** of the provider's response, **the api-docs claims are unverified inputs, not ground truth.** At brief-draft time, for any wire-shape claim the brief is about to lock — URL templates, layer names, field paths, header names, content types — capture a fresh live response and cross-check the api-docs claim against the actual wire shape.

**How to apply:**

- For api-docs files you cite at brief-draft time: check the file header for a "Captured: YYYY-MM-DD via <live URL>" line documenting capture provenance. If absent, do a live capture before locking the brief OR mark the unverified claim explicitly in the brief as "tentative, verify at fixture-capture time" so test-author flags it earlier.
- For api-docs files you write yourself when the cross-check rule fires on a new provider: include a "Captured: YYYY-MM-DD via <live URL>" line for every wire-shape claim that came from a live response. If you only have documentation (no live access at writing time), explicitly mark the claim as "per upstream docs, not live-verified."
- This rule does NOT require re-capturing api-docs files at every round. Once an api-docs file has a "Captured: ..." header, the brief can trust it until the provider's external API changes (which is documented separately when an API change is detected).

**Why (2026-05-11):** Phase 2 task 3b-14 close — 3 of the 5 WMS-T radar provider api-docs files (`noaa_mrms`, `msc_geomet`, `dwd_radolan`) were written from web research at brief-draft time, NOT from live `GetCapabilities` captures. The layer names in the api-docs (`"0"`, `RADAR_1KM_RDPR`, `dwd:RX-Produkt`) were either guesses (NOAA `"0"` was from "ArcGIS image services typically expose layer id 0") or stale references that did NOT exist in live capabilities (`RADAR_1KM_RDPR` returns "Layer not available"; `dwd:RX-Produkt` is absent from current GeoServer). test-author detected the divergence during fixture capture (real `GetCapabilities` showed different layer names) and INJECTED synthetic layer names into fixtures so tests would pass against the impl — surfacing the bugs through closeout items B2-B4. Lead-direct fix in `f2362ee` corrected the three `LAYER_NAME` constants AND updated `api-docs/{noaa_mrms,msc_geomet,dwd_radolan}.md` to record the real layer names with capture provenance. Cost was ~50 lines of impl + test + docs edits across 12 files. If api-docs claims had been validated against live captures at brief-draft (or the brief had marked them tentative), the bugs would not have shipped.

**Anti-pattern:** treating an api-docs file as authoritative when its claims came from upstream documentation that wasn't live-verified. Upstream documentation drifts — vendor pages get rewritten, layer names get renamed, conventions documented for one product version don't apply to another. The only ground truth is a live response captured today.
