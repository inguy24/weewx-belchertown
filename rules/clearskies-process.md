...
- The repo files live on DILBERT (this workspace). Sync into the container via the agreed mechanism — git clone the meta repo and child repos into the container, OR (Phase 1 setup) bind-mount the workspace from the ratbert host into the container. Pick once at container-stand-up time and document.
